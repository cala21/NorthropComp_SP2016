...
from keras.layers import Input, merge, Convolution2D, MaxPooling2D, UpSampling2D, Reshape, core, Dropout, Convolution1D, MaxPooling1D, UpSampling1D
...
def get_1d(n_ch,patch_height,patch_width):
    inputs = Input((n_ch, patch_height, patch_width))
    conv1 = Convolution1D(32, 3, activation='relu', border_mode='same')(inputs)
    conv1 = Dropout(0.2)(conv1)
...
patches_imgs_train, patches_masks_train, patches_imgs_test, patches_masks_test = db.getTestAndTrainingData(batches=batch_size)
'''
 get_data_training(
